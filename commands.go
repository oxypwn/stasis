...
func getHostState(host Host, store Store, hostListItems chan<- hostListItem) {
	...
	hostListItems <- hostListItem{
		...
		PermitPreinstall: host.PermitPreinstall,
		Install:          host.Install,
		PermitInstall:    host.PermitInstall,
		Status:           host.Status,
	...
	},
	{
		Flags: []cli.Flag{
			cli.StringFlag{
				Name:   "addr, a",
				Value:  "localhost",
				Usage:  "default address to listen on",
				EnvVar: "STASIS_HTTP_ADDR",
			},
			cli.StringFlag{
				Name:   "port",
				Value:  "8080",
...
func cmdCreateHost(c *cli.Context) {
	...
	host, err := store.CreateHost(name, storePath, mac, preinstall, install, username, password, postinstall, windowsKey, append, mirror, kernel, initrd, status, permitpreinstall, permitinstall, allowpostinstall, announce)
	if err != nil {
		log.Fatal(err)
	}
...
func cmdLs(c *cli.Context) {
	...
	if !quiet {
		fmt.Fprintln(w, "NAME\tACTIVE\tSTATUS\tPREINSTALL")
	}
	...
	for _, item := range items {
		...
		fmt.Fprintf(w, "%s\t%s\t%s\t%t\n",
			item.Name, activeString, item.Status, item.PermitPreinstall)
	}
...
func cmdListen(c *cli.Context) {
	...
	os.Setenv("STASIS_HTTP_ADDR", c.String("addr"))
	store := NewHostStore(c.GlobalString("storage-path"))
